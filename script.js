...
function startVideo(){
  ...
  stream => imageUpload.srcObject = stream,
...
async function start() {
  ...
  imageUpload.addEventListener('click', async () => {
    ...
    document.getElementById('imageUpload').append(canvas)
...
function loadLabeledImages() {
  const labels = ['Renzo', 'Captain America', 'Captain Marvel', 'Hawkeye', 'Jim Rhodes', 'Thor', 'Tony Stark']
  return Promise.all(
...
function loadLabeledImages() {
  ...
  return Promise.all(
    labels.map(async label => {
      ...
      // }
>>>>>>> 52f074121fa9898a8847d5befbc1b2207d9a4c84
